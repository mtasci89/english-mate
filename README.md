...
- `npm run build:audio` runs `scripts/build-audio.mjs` **locally**, once
  ...
  from `src/curriculum/*.json` to `public/audio/<key>.mp3`, using one
  consistent voice for English lines and a different consistent voice for
  Turkish lines, and writes `public/audio/manifest.json` — a JSON array of
  every key that has a file. It is idempotent (skips a key whose file
